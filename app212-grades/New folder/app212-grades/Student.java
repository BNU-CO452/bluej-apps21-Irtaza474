...
public class Student
{
    ...
    public void awardMark(String moduleCode, int value)
    {
        for(int i=0; i< this.course.modules.size() ; i++)
        {
            ...
            {
                ModuleMark mmobj= new ModuleMark(this.course.modules.get(i));
                mmobj.setMark(value);
                marks.add(mmobj);
                //System.out.println("awardMark working");
            }
        }
    }
    ...
    public void awardTestMarks()
    {
        ...
        for(int i=0; i< this.course.modules.size(); i++)
        {
            awardMark(this.course.modules.get(i).getCode(), random.nextInt(100));
        }
    }
    ...
    private void printModules()
    {
        for(int i=0; i< this.course.modules.size(); i++)
        {
            Module obj= this.course.modules.get(i);
            System.out.println(" " + obj.getCode()+" \t "
            +obj.getTitle()+" \t "
            + this.marks.get(i).getCredit() + " \t " 
            + this.marks.get(i).getValue() + " \t " 
            + course.convertToGrade(this.marks.get(i).getValue()));
        }
    }
